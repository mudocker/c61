			            ...
			            <div class="panel-body">
			                <div class="activity_content">
			                	<div class="activity_info1">
									...
									<p>系统维护升级,晋级奖励暂停,敬请期待...</p>
<!--			                		<p>
			                			<span>当前等级：<em>{$userinfo.groupname}</em></span>
										<if condition="$userinfo.groupid neq '10'">
										<span>上次晋级等级：<em><empty name="jjlist[0]['groupname']">VIP1<else/>{$jjlist[0].groupname}</empty></em></span>
									...
									<table class="table table-bordered ">
										...
										<volist name="jjlist" id="vo">
											<tr><td>{$vo.oddtime|date="m-d H:i",###}</td>
												...
												<td>{$vo.point}</td>
												<td>{$vo.jlje}</td>
												<td><if condition="$vo['shenhe'] eq 0"><span style="color:grey">审核中</span><elseif condition="$vo['shenhe'] eq 1"/><span style="color:green">通过</span><elseif condition="$vo['shenhe'] eq -1"/><span style="color:green">未通过</span></if></td></tr>
										</volist>
										</tbody>
									</table>
								</notempty>
