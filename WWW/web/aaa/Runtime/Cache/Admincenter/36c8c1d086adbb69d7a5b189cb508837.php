	...
	<form action="" method="post" class="form form-horizontal validate-form" id="AjaxPostForm">
    	...
    	<span class="select-box inline">
            <select name="parentid" class="select">
                ...
                <?php if(is_array($catelist)): $i = 0; $__LIST__ = $catelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cat): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cat["id"]); ?>" <?php if($cat['id'] == $info['parentid']): ?>selected<?php endif; ?> ><?php echo ($cat["spacer"]); echo ($cat["catname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
		</span>
			</div>
		</div>
        ...
        <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($info["catname"]); ?>" placeholder="栏目名称" name="catname">
			</div>
		</div>
