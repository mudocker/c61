...
<a href="javascript:;" layer-url="<?php echo U('add');?>" title="添加管理员" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加q管理员</a>&nbsp;&nbsp;&nbsp;
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td><u style="cursor:pointer" class="text-primary" layer-url="<?php echo U('edit',['id'=>$vo['id']]);?>" title="编辑-<?php echo ($vo["username"]); ?>"><?php echo ($vo["username"]); ?></u></td>
				<td class="td-manage">
                <u style="cursor:pointer" class="text-primary" layer-url="<?php echo U('edit',['id'=>$vo['id']]);?>" title="编辑-<?php echo ($vo["username"]); ?>">修改</u> | <u style="cursor:pointer" class="text-primary <?php if($vo[islock] == 0): ?>c-999<?php elseif($vo[islock] == 1): ?>c-green<?php endif; ?>" lock-url="<?php echo U('lock',['id'=>$vo['id']]);?>" title="锁定/解锁-<?php echo ($vo["username"]); ?>"><?php if($vo['islock'] == 0): ?>锁定<?php elseif($vo['islock'] == 1): ?>解锁<?php endif; ?></u>
                |
                <a style="text-decoration:none" class="ml-5" layer-del-url="<?php echo U('delete',array('id'=>$vo['id']));?>" href="javascript:;" title="删除">删除</a>
                </td>
