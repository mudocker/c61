    ...
    <div class="mt-5">
        <form method="post" action="<?php echo U(CONTROLLER_NAME.'/'.ACTION_NAME);?>">
            <table class="table table-border table-bordered table-hover table-bg table-sort">
                ...
                <tbody>
                <?php if(is_array($jiangli)): $i = 0; $__LIST__ = $jiangli;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                    <td width="80"><?php echo ($vo["uid"]); ?></td>
                    <td width="80"><?php echo ($vo["username"]); ?></td>
                    <td width="60"><?php echo ($vo["beforegroupname"]); ?></td>
                    <td width="60"><?php echo ($vo["groupname"]); ?></td>
                    <td width="60"><?php echo ($vo["point"]); ?></td>
                    <td width="60"><?php echo ($vo["jlje"]); ?>元</td>
                    <td width="60"><?php echo (date('Y-m-d',$vo["oddtime"])); ?></td>
                    <td width="30">
                        <?php if($vo['shenhe'] == 0): ?><u style="cursor:pointer;color:red" class="text-primary" layer-url="<?php echo U('jinjishenhe',['id'=>$vo['id']]);?>" title="会员晋级审核">未审核</u>
                        ...
                        未知<?php endif; ?>
                        |
                      <!--  <a style="text-decoration:none" class="ml-5" layer-del-url="<?php echo U('jinjijianglidelete',array('id'=>$vo['id']));?>" href="javascript:;" title="删除">删除</a>-->
                    </td>
