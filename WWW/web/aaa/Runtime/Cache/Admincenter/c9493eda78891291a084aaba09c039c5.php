...
<div class="page-container">
	<div class="text-c">
    	<form method="get" action="<?php echo U(CONTROLLER_NAME.'/'.ACTION_NAME);?>">
<!--<span class="select-box inline">
            <select name="moduleid" class="select">
                <option value="0">==选择模型==</option>
                <?php if(is_array($modulelist)): $i = 0; $__LIST__ = $modulelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$md): $mod = ($i % 2 );++$i;?><option value="<?php echo ($md["name"]); ?>" <?php if($md['name'] == $moduleid): ?>selected<?php endif; ?>><?php echo ($md["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
		</span>-->
    	<span class="select-box inline">
            <select name="catid" class="select">
                <option value="0">==选择栏目==</option>
                <?php if(is_array($catelist)): $i = 0; $__LIST__ = $catelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cat): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cat["id"]); ?>" <?php if($cat['id'] == $catid): ?>selected<?php endif; ?> ><?php echo ($cat["spacer"]); echo ($cat["catname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
		</span>
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a class="btn btn-primary radius" href="javascript:;" layer-url="<?php echo U('add',['catid'=>$catid]);?>" title="添加信息"><i class="Hui-iconfont">&#xe600;</i> 添加信息</a></span> <span class="r"><div class="l pages"><?php echo ($pageshow); ?></div>共有数据：<strong><?php echo ($totalcount); ?></strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td><?php echo (date('y-m-d H:i',$vo["oddtime"])); ?></td>
                    <td class="td-manage">
                    <a style="text-decoration:none" class="ml-5" layer-url="<?php echo U('edit',['id'=>$vo['id']]);?>" title="修改信息">修改</a>
                    
                    <a style="text-decoration:none" class="ml-5" layer-del-url="<?php echo U('delete',['id'=>$vo['id']]);?>" href="javascript:;" title="删除">删除</a>
                    </td>
