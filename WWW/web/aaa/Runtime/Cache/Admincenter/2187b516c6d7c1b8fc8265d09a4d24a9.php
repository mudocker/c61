...
<a href="javascript:;" layer-url="<?php echo U('bankadd');?>" title="添加提款银行" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加提款银行</a>&nbsp;&nbsp;&nbsp;
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td><u style="cursor:pointer" class="text-primary" layer-url="<?php echo U('bankedit',['id'=>$vo['id']]);?>" title="修改-<?php echo ($vo["bankname"]); ?>"><?php echo ($vo["bankname"]); ?></u></td>
				<td><?php echo ($vo["bankname"]); ?></td>
				<td class="td-status">
                <?php if($vo['state'] == 1): ?><span class="label label-success radius" status-url="<?php echo url('setstatus',['id'=>$vo['id'],'name'=>'state']);?>">启用</span>
                <?php else: ?>
                <span class="label label-defaunt radius" status-url="<?php echo url('setstatus',['id'=>$vo['id'],'name'=>'state']);?>">禁用</span><?php endif; ?>
                
                </td>
				<td class="td-manage">
                <a style="text-decoration:none" class="ml-5" layer-url="<?php echo U('bankedit',['id'=>$vo['id']]);?>" title="修改-<?php echo ($vo["bankname"]); ?>"><i class="Hui-iconfont">&#xe6df;</i></a>
