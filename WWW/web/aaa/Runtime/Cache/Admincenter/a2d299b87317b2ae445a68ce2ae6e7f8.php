...
<a href="javascript:;" layer-url="<?php echo U('paysetadd');?>" title="添加存款方式" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加存款方式</a>&nbsp;&nbsp;&nbsp;
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td><input type="number" class="input-text radius size-S" value="<?php echo ($vo["listorder"]); ?>" name="listorder[<?php echo ($vo["id"]); ?>]" style="width:60px;"></td>
                <td><?php echo ($vo["paytype"]); ?></td>
				<td><u style="cursor:pointer" class="text-primary" layer-url="<?php echo U('paysetedit',['id'=>$vo['id']]);?>" title="修改-<?php echo ($vo["paytypetitle"]); ?>"><?php echo ($vo["paytypetitle"]); ?></u></td>
                <td><?php echo ($vo["ftitle"]); ?></td>
				<td>
                <?php if($vo['isonline'] == 1): ?><span class="label label-success radius">是</span>
                <?php else: ?>
                <span class="label label-defaunt radius">否</span><?php endif; ?>
                
                </td>
				<td class="td-status">
                <?php if($vo['state'] == 1): ?><span class="label label-success radius" status-url="<?php echo url('paysetstatus',['id'=>$vo['id'],'name'=>'state']);?>">启用</span>
                <?php else: ?>
                <span class="label label-defaunt radius" status-url="<?php echo url('paysetstatus',['id'=>$vo['id'],'name'=>'state']);?>">禁用</span><?php endif; ?>
                
                </td>
				<td class="td-manage">
                <u style="cursor:pointer" class="text-primary" layer-url="<?php echo U('paysetedit',['id'=>$vo['id']]);?>" title="修改-<?php echo ($vo["paytypetitle"]); ?>">修改</u>&nbsp;|&nbsp;<u style="cursor:pointer" class="text-primary" layer-del-url="<?php echo U('paysetdelete',['id'=>$vo['id']]);?>" title="删除-<?php echo ($vo["paytypetitle"]); ?>">删除</u>
                </td>
			</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		</tbody>
	</table>
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" listorder-url="<?php echo U('paysetlistorder');?>" title="排序" class="btn btn-danger-outline radius">排序</a></span> <span class="r">共有数据：<strong><?php echo count($olist);?></strong> 条</span> </div>
    </form>
	</div>
</div>
<!--_footer 作为公共模版分离出去-->
