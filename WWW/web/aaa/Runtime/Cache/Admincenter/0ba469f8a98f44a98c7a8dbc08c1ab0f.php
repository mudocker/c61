	...
	<form action="" method="post" class="form form-horizontal validate-form" id="AjaxPostForm">
    	...
    	<span class="select-box inline">
            <select name="catid" class="select">
                ...
                <?php if(is_array($catelist)): $i = 0; $__LIST__ = $catelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cat): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cat["id"]); ?>" <?php if($cat['id'] == $info['catid']): ?>selected<?php endif; ?> ><?php echo ($cat["spacer"]); echo ($cat["catname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
		</span>
			</div>
		</div>
        ...
        <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($info["title"]); ?>" placeholder="标题" name="title">
			</div>
		</div>
        <div class="row cl">
		...
		<textarea id="content" style="" name="content" ><?php echo ($info["content"]); ?></textarea>
<?php
$URL_PREFIX = is_ssl()?'https://':'http://'; $uploadJson = $URL_PREFIX.$_SERVER["HTTP_HOST"].U('Uploads/upload',array('allowpath'=>'1','allowext'=>'gif|jpg|jpeg|png|bmp')); $fileManagerJson = U('Uploads/file_manager'); ?>
		<script>
		var editor;
		...
		width : "100%",
		height : "80px",
		uploadJson: "<?php echo ($uploadJson); ?>",
		fileManagerJson : "<?php echo ($fileManagerJson); ?>",
