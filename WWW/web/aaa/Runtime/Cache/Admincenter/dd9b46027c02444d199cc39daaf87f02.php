...
<a href="javascript:;" layer-url="<?php echo U('categoryadd');?>" title="添加栏目" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加栏目</a>&nbsp;&nbsp;&nbsp;
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				<td><input type="checkbox" class="checkids" value="<?php echo ($vo[$_pk]); ?>" name="ids[<?php echo ($vo[$_pk]); ?>]"></td>
				<td><input type="number" class="input-text radius size-S" value="<?php echo ($vo["listorder"]); ?>" name="listorder[<?php echo ($vo[$_pk]); ?>]" style="width:60px;"></td>
                <td><?php echo ($vo["id"]); ?></td>
                <td class="text-l"><?php echo ($vo["spacer"]); echo ($vo["catname"]); ?></td>
				<td class="td-manage">
                <a style="text-decoration:none" class="ml-5" layer-url="<?php echo U('categoryedit',['id'=>$vo['id']]);?>" title="修改-<?php echo ($vo["bankname"]); ?>"><i class="Hui-iconfont">&#xe6df;</i></a>
                
                <a style="text-decoration:none" class="ml-5" layer-del-url="<?php echo U('categorydelete',['id'=>$vo['id']]);?>" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
                </td>
			</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		</tbody>
	</table>
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" listorder-url="<?php echo U('categorylistorder');?>" title="排序" class="btn btn-danger-outline radius">排序</a></span> <span class="r">共有数据：<strong><?php echo count($list);?></strong> 条</span> </div>
    </form>
	</div>
</div>
<!--_footer 作为公共模版分离出去-->
