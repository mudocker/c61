...
<div class="page-container">
	<form method="get" action="<?php echo U('transrecord');?>" class="text-c">

转换类型：&nbsp;&nbsp;&nbsp;&nbsp;<span class="select-box" style="width:120px">
<select class="select" name="transType">
<option value="">全部</option>
<option value="ag" <?php if($transType == 'ag'): ?>selected<?php endif; ?>>AG</option>
<option value="bb" <?php if($transType == 'bb'): ?>selected<?php endif; ?>>BBIN</option>
</select>
</span>&nbsp;&nbsp;&nbsp;转让时间:&nbsp;&nbsp;&nbsp;<input class="input-text" type="text" style="width:180px;" 
onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="sDate" value="<?php echo ($_sDate); ?>"> - 
<input class="input-text" type="text" style="width:180px;" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<?php echo ($_eDate); ?>" name="eDate">&nbsp;&nbsp;

        用户名：<input class="input-text" type="text"  value="<?php echo ($username); ?>" name="username" style="width:180px">
        
        <a class="btn btn-default-outline radius" href="<?php echo U('transrecord');?>">重置</a>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td><?php echo ($vo["transID"]); ?></td>
               
                <td><?php echo ($vo["username"]); ?></u></td>
              
                 <td><?php echo ($vo["transBillno"]); ?></td>
                <td><?php echo ($vo["transType"]); ?></td>
            
				<td><?php echo ($vo["transDes"]); ?></td>
				<td><?php echo ($vo["tansAmount"]); ?></td>
                
                <td><?php echo ($vo["transTime"]); ?></td>
