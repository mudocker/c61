...
<head>
    ...
    <![endif]-->
    <title><?php echo GetVar('webtitle');?>后台管理</title>
</head>
<body>
<!--<div class="header">梦想后台管理系统</div>-->
<div class="loginWraper">
    <div id="loginform" class="loginBox">
        ...
        <form class="form form-horizontal" action="" method="post">
            ...
            <?php if(GetVar('islogincode') == 1): ?><div class="row cl">
                <label class="form-label col-xs-3"><i class="Hui-iconfont">&#xe63f;</i></label>
                <div class="formControls col-xs-8">
                    <input id="code" name="info[code]" class="input-text size-L" type="text" placeholder="图像验证码" style="width:150px;">
                    <img id="verifycode" src="<?php echo url('Public/verify',['imageW'=>100,'imageH'=>39,fontSize=>14]);?>" onclick="this.src=this.src+'?temp='+ 1" title="看不清，换一张"></div>
            </div><?php endif; ?>

            <?php if(GetVar('isemailcode') == 1): ?><div class="row cl">
                <label class="form-label col-xs-3"><i class="Hui-iconfont">&#xe70b;</i></label>
                <div class="formControls col-xs-8">
...
<script>
    function sendcode(obj){
        ...
        $.post("<?php echo U('Public/sendcode');?>",{'act':'senddo','username':$("#name").val()}, function(json){
            if(json.status==1){
                alert(json.info);
                settime(obj);
    ...
    var countdown = countdown1 = parseInt("<?php echo GetVar('adminemailcodetime');?>");
