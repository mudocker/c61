...
<div class="page-container">
  ...
  <div class="tabBar cl"><span>会员组[-<?php echo ($info["groupname"]); ?>-]限额设置</span><small>&nbsp;&nbsp;最低投注为0则以玩法设置为准&nbsp;&nbsp;&nbsp;&nbsp;最高投注为0则不限制</small></div>
    <table class="table table-border table-bordered table-hover table-bg table-sort">
        ...
        <tbody>
            <?php if(is_array($getplayers)): $i = 0; $__LIST__ = $getplayers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                <th><?php echo ($vo["title"]); ?></th>
                <td>
                <input type="text" name="configs[min_<?php echo ($vo["playid"]); ?>]" class="input-text" value="<?php echo $configs['min_'.$vo['playid']];?>" >
                </td>
                <td><input type="text" name="configs[max_<?php echo ($vo["playid"]); ?>]" class="input-text" value="<?php echo $configs['max_'.$vo['playid']];?>" ></td>
	...
	<br>
    <input name="id" type="hidden" value="<?php echo ($info["groupid"]); ?>">
