...
<option value="0" <?php if(($state == 0) and (isset($state))): ?>selected<?php endif; ?>>审核中</option>
<option value="1" <?php if($state == 1): ?>selected<?php endif; ?>>已审</option>
<option value="2" <?php if($state == 2): ?>selected<?php endif; ?>>驳回</option>
</select></span>
        用户名：<input class="input-text" type="text" style="width:100px;" value="<?php echo ($username); ?>" name="username">
        绑定姓名：<input class="input-text" type="text" style="width:100px;" value="<?php echo ($accountname); ?>" name="accountname">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td><?php echo ($vo["bankbranch"]); ?></td>
                <td><?php echo ($vo["bankaddress"]); ?></td>
                <td><?php echo ($vo["banknumber"]); if($vo[isdefault] == 1): ?>(默认)<?php endif; ?></td>
                <td>
                ...
                <td><u style="cursor:pointer" class="text-primary" layer-url="<?php echo U('bankedit',['id'=>$vo['id']]);?>" title="修改银行信息">修改</u>&nbsp;|&nbsp;<u style="cursor:pointer" class="text-primary" layer-del-url="<?php echo U('bankdelete',['id'=>$vo['id']]);?>" title="删除银行信息">删除</u></td>
