...
<a class="btn btn-success radius r btn-refresh" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a>
    ...
    <form method="get" action="<?php echo U(CONTROLLER_NAME.'/'.ACTION_NAME);?>" class="text-c">
        状态：<span class="select-box" style="width:100px"><select class="select" name="shenhe">
                ...
                <option value="1" <?php if($_GET['shenhe']== '1'): ?>selected<?php endif; ?>>已发放</option>
                <option value="0" <?php if($_GET['shenhe']== '0'): ?>selected<?php endif; ?>>未发放</option>
                <option value="-1" <?php if($_GET['shenhe']== '-1'): ?>selected<?php endif; ?>>未通过</option>
            </select></span>
        订单号：<input class="input-text" type="text" style="width:150px;" value="<?php echo ($_GET['trano']); ?>" name="trano">
        用户名：<input class="input-text" type="text" style="width:150px;" value="<?php echo ($_GET['username']); ?>" name="username">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
        <?php if(is_array($dailiinfo)): $i = 0; $__LIST__ = $dailiinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td width="80"><?php echo ($vo["trano"]); ?></td>
                <td width="50"><?php echo ($vo["touzhujine"]); ?></td>
                <td width="50"><?php echo ($vo["amount"]); ?></td>
                <td width="50"><?php echo ($vo["fandian"]); ?></td>
                <td width="50"><?php echo ($vo["xiajiuser"]); ?></td>
                <td width="50"><?php echo ($vo["xiajifandian"]); ?></td>
                <td width="60"><?php echo (date('Y-m-d',$vo["oddtime"])); ?></td>
                <td width="50">
                    <?php if($vo['shenhe'] == 0): ?><u style="cursor:pointer;color:red" class="text-primary" layer-url="<?php echo U('yongjinshehe',['id'=>$vo['id']]);?>" title="代理佣金发放">未发放</u>
                    <?php elseif($vo['shenhe'] == 1): ?>
                    ...
                    未知<?php endif; ?>
                |
                <a style="text-decoration:none" class="ml-5" layer-del-url="<?php echo U('yongjindelete',array('id'=>$vo['id']));?>" href="javascript:;" title="删除">删除</a>
                </td>
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20 text-c">
        <div class="l" style="padding:0"><a href="javascript:;" layer-alt-url="<?php echo U('yongjinshehes');?>" title="发放" class="btn btn-danger-outline radius">发放</a>&nbsp;&nbsp;</div>
        <div class="l" style="padding:0"><a href="javascript:;" deleteall-url="<?php echo U('yongjindeleteall');?>" title="删除" class="btn btn-danger-outline radius">删除</a></div>
        <div class="r">
...
<script type="text/javascript" src="/aaa/Template/admin/resources/ui/lib/bootstrap-modal/2.2.4/bootstrap-modalmanager.js"></script>
<script type="text/javascript" src="/aaa/Template/admin/resources/ui/lib/bootstrap-modal/2.2.4/bootstrap-modal.js"></script>
<script>
