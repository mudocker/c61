...
<?php if(is_array($cpcategory)): foreach($cpcategory as $cpk=>$cptype): ?><optgroup label="<?php echo ($cptype); ?>">
    <?php if(is_array($cplist)): $i = 0; $__LIST__ = $cplist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cpv): $mod = ($i % 2 );++$i; if($cpk == $cpv['typeid']): ?><option value="<?php echo ($cpv['name']); ?>" <?php if($cpv['name'] == $name): ?>selected<?php endif; ?>><?php echo ($cpv["title"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
