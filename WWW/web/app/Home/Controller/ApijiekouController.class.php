...
//		exit(dump($Result));
	...
	function cpbuy(){
		...
		if(strstr($_REQUEST['lotteryname'],'ssc')){
			...
			$rates = C('rates_pk10');
		}elseif(strstr($_REQUEST['lotteryname'],'keno')){
			$rates = C('rates_keno');
		}elseif(strstr($_REQUEST['lotteryname'],'x5')){
			$rates = C('rates_x5');
		}elseif(strstr($_REQUEST['lotteryname'],'lhc')){
			$rates = C('rates_lhc');
		}elseif(strstr($_REQUEST['lotteryname'],'pl3') or strstr($_REQUEST['lotteryname'],'fc3d') or strstr($_REQUEST['lotteryname'],'df3d')){
			$rates = C('rates_dpc');
		}else{
			$rates = C('rates_k3');
		}
	...
	function userbindbankcard(){
		...
		if(md5(sha1($bankTradPwd))!=$userinfo['tradepassword'] && $userinfo['tradepassword']){
			echo jsonreturn(['sign'=>false,'message'=>'资金密码错误']);exit;
		}
		$apiparam = [];
	...
	function usersecurity(){
		...
		if(!preg_match($passwordpatten,$tradePwd)){
			echo jsonreturn(['sign'=>false,'message'=>'资金密码应为4-16位字符']);exit;
		}
	...
	function usereditdrawpass(){
		...
		if(!preg_match($passwordpatten,$oldpassword)){
			echo jsonreturn(['sign'=>false,'message'=>'旧资金密码应为4-16位字符']);exit;
		}
		if(!preg_match($passwordpatten,$password)){
			echo jsonreturn(['sign'=>false,'message'=>'新资金密码应为4-16位字符']);exit;
		}
		if($password!=$rpassword){
			echo jsonreturn(['sign'=>false,'message'=>'新资金密码两次输入不一致']);exit;
		}
		if(!$userinfo['tradepassword']){
			$userinfo['tradepassword'] = $userinfo['password'];
		}
		if(md5(sha1($oldpassword))!=$userinfo['tradepassword']){
			echo jsonreturn(['sign'=>false,'message'=>'旧资金密码错误']);exit;
		}
		$apiparam=array();
	...
	function userbindrealname(){
		...
		if($userinfo['tradepassword']==''){
			echo jsonreturn(['sign'=>false,'message'=>'请先设置资金密码']);exit;
		}
...
/*		//验证用户名
		...
		//验证用户名
		$_paten = "/\/|\~|\!|\@|\#|\\$|\%|\^|\&|\*|\(|\)|\_|\+|\{|\}|\:|\<|\>|\?|\[|\]|\,|\.|\/|\;|\'|\`|\-|\=|\\\|\|/";
		if(!$username || preg_match($_paten,$username) || mb_strlen($username)<2 || mb_strlen($username,'utf-8')>12){
			$Result = [];
			$Result['sign']    = false;
	...
	function adduser(){
		...
		if(preg_match("/\/|\~|\!|\@|\#|\\$|\%|\^|\&|\*|\(|\)|\_|\+|\{|\}|\:|\<|\>|\?|\[|\]|\,|\.|\/|\;|\'|\`|\-|\=|\\\|\|/",$username)){
			$return = ['sign'=>false,'message'=>"用户名格式5-10位字母或数字,字母开头"];
			echo json_encode($return, JSON_UNESCAPED_UNICODE);exit;
		}
		if(M('member')->where(['username'=>$username])->getField('id')){
			$return = ['sign'=>true,'message'=>"注册用户名已经存在,请更换..."];
			echo json_encode($return, JSON_UNESCAPED_UNICODE);exit;
		}
/*		//验证用户名
		if(!$username || !preg_match("/^[a-zA-Z][a-zA-Z\d]{4,11}$/",$username)){
	...
	function savepointchangemoney(){
		...
		if(!$tradepassword || md5(sha1($tradepassword))!=$userinfo['tradepassword']){
			$return =['sign'=>false,'message'=>"资金密码错误"];
			echo json_encode($return, JSON_UNESCAPED_UNICODE);exit;
		}
	...
	function checkrechargeisok1(){
		$trano   = strip_tags(I('trano'));
		$state = M('recharge')->where(['trano'=>$trano])->getField('state');
//		echo json_encode(['sign'=>true,'state'=>$state]);
		$this->ajaxReturn(['sign'=>true,'state'=>$state]);
	}
}
?>