...
/*	function getids(){
	...
	function downuserbetslist($apiparam=array()){
		...
		$map = [];
		if($chiduids)$map['uid'] = ['in',$chiduids];
		if($startime){
			$map['oddtime'][] = ['egt',$startime];
		}
		if($endtime){
		...
		if($chiduids){
			$map = [];
			$map['uid'] = ['in',$chiduids];
			if($startime)$map['oddtime'][] = ['egt',$startime];
			if($endtime)$map['oddtime'][] = ['elt',strtotime(date('Y-m-d 23:59:59',$endtime))];
			if($trano)$map['trano'] = ['eq',$trano];
			if($lotteryname)$map['cpname'] = ['eq',$lotteryname];
			if($expect)$map['expect'] = ['eq',$expect];
			if($state!='' && in_array($state,[0,1,-1,-2]))$map['isdraw'] = ['eq',$state];
