...
//晋级
	function promotion()
	{
		...
		{		$map['uid'] = $this->userinfo['id'];
			...
			{   //如果为空则从普通会员算起,
				$jlje = M('membergroup')->where("groupid = '{$this->userinfo['groupid']}'")->field('groupid,tiaoji')->find();
...
//						if(($user['groupid']-$jiangli[0]['groupid'])>=2)$amount++;
...
//					if(($user['groupid']-$jiangli[0]['groupid'])>=2)$amount++;
	...
	function activitylist1(){
		$otherhd = M('News')->cache(300)->where("id='61'")->select();
		$this->assign('houdong',$otherhd[0]['content']);
		$this->display();
	}
	function activitylist2(){
		$otherhd = M('News')->cache(300)->where("id='62'")->select();
		$this->assign('houdong',$otherhd[0]['content']);
		$this->display();
	}
	function activitylist3(){
		$otherhd = M('News')->cache(300)->where("id='63'")->select();
		$this->assign('houdong',$otherhd[0]['content']);
		$this->display();
	}
	function activitylist4(){
		$otherhd = M('News')->cache(300)->where("id='64'")->select();
		$this->assign('houdong',$otherhd[0]['content']);
		$this->display();
	}
	function activitylist5(){
		$otherhd = M('News')->cache(300)->where("id='65'")->select();
		$this->assign('houdong',$otherhd[0]['content']);
		$this->display();
	}
