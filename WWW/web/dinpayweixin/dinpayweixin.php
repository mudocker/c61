...
class dinpayweixin{
	...
	function get_code($order,$configs,$host){
		...
		if(is_file($pic) && $order['state']==0){
			$html = '';
			$html .= "<center>请使用微信扫描下面的二维支付码1</center><br>";
			$html .= "<center><img src='".ltrim($pic,'.')."'></center>";
			return $html;
			exit;
		}

		include ('./dinpayweixin/phpqrcode.php');
		/* *
         *功能：智付微信支付接口
         *版本：3.0
         *日期：2016-07-10
...
///////////////////////////  初始化接口参数  //////////////////////////////
		...
		$notify_url = $configs['returnbackurl'].'/Payback.dinpayweixin';	//异步地址
...
//			$QRcode->png ($qrcode, "'.$pic.'", $errorCorrectionLevel, $matrixPointSize, 2 );
			QRcode::png ( $qrcode, $pic, $errorCorrectionLevel, $matrixPointSize, 2 );
			$html = '';
			$html .= "<center>请使用微信扫描下面的二维支付码</center><br>";
