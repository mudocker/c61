	  ...
	  $.post(url, {'lotteryname':lotteryname}, function(data) {
		...
		if(data.sign==true){
			...
			for(var o in lotlist){
				...
				var lhckaijList = lhcMobileKaiHtml(array);
        var yopentimes = format(openinfo.opentime * 1000);
        var yopentimes_arr = yopentimes.split(' ');
        var yopentime = yopentimes_arr[1];
				html += '<tr><td>'+openinfo.expect+'</td><td class="c_red fb" style="padding:5px;width: 148px;">'+lhckaijList+'</td><td width="40" class="c_blue fb" style="padding:5px;">'+yopentime+'</td></tr>';
	...
	clearTimeout(openCodeTimeOut);
	 $.ajax({  
         ...
         success:function(msg){ 
					 var msg =  {"lotteryname":"lhc","expect":"20171010026","sign":true,"message":"获取成功！","data":{"name":"cqssc","title":"六合彩","opencode":"45,48,03,38,06,24,05","sourcecode":"","remarks":"","expect":"20171010026","opentime":"1507602000"}}
            if (msg.sign == true) {
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
						...
						var lhckaijList = lhcMobileKaiHtml(openCodes);
                        var sum = parseInt(openCodes[0])+parseInt(openCodes[1])+parseInt(openCodes[2]);
                        var yopentimes = format(msg.data.opentime * 1000);
                        var yopentimes_arr = yopentimes.split(' ');
                        var yopentime = yopentimes_arr[1];
                        var html = '<tr><td class="lsqh">' + openexpect + '</td><td style="padding:5px;width: 148px;" class="c_red fb';
                        html += '">';
                        html += lhckaijList;
